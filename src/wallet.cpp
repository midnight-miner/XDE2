...
// XDE2: total coins staked (non-spendable until maturity)
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // Calculate coin age reward
    {
        ...
        int64_t nReward = GetProofOfStakeReward(nCoinAge, nFees, pDestination);
        if (nReward <= 0)
            return false;
...
// XDE2: check 'spent' consistency between wallet and txindex
// XDE2: fix wallet spent state according to txindex
...
// XDE2: disable transaction (only for coinstake)
