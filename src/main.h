...
static const int64_t MAX_MONEY = 200000 * COIN; // 200000 XDE2
static const int FORK_HEIGHT = 2000;
...
#endif
static const uint256 hashGenesisBlock("0x000007a0132410ecc653c8ecf81cef94699c0de79e3a412107d7d6bd2589a2b8");
static const uint256 hashGenesisBlockTestNet("0x");

inline int64_t PastDrift(int64_t nTime)   { return nTime - 2 * 60; } // up to 2 minutes from the past
inline int64_t FutureDrift(int64_t nTime) { return nTime + 2 * 60; } // up to 2 minutes from the future
...
public:
    ...
    {
        // XDE2: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // XDE2: get transaction coin age
...
public:
    ...
    // XDE2: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // XDE2: two types of block: proof-of-work or proof-of-stake
    ...
    // XDE2: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // XDE2: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // XDE2: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // XDE2: block index flags
    enum
    {
